...
public sealed class IdentityGateway : IIdentityGateway
{
    ...
    public async Task<IdentityUserDto?> FindByEmailAsync(string email, CancellationToken ct = default)
    {
        ...
        return user is null ? null : new IdentityUserDto(user.Id, user.Email ?? email);
    }

    public async Task<IdentityUserDto?> FindByIdAsync(string userId, CancellationToken ct = default)
    {
        var user = await _userManager.FindByIdAsync(userId);
        return user is null ? null : new IdentityUserDto(user.Id, user.Email ?? string.Empty);
    }
    ...
    public async Task<IReadOnlyList<Claim>> GetClaimsAsync(string userId, CancellationToken ct = default)
    {
        var user = await _userManager.FindByIdAsync(userId);
    ...
    public async Task<Result> ChangePasswordAsync(string email, string currentPassword, string newPassword, CancellationToken ct = default)
    {
        var user = await _userManager.FindByEmailAsync(email);
        if (user is null)
            return Result.Fail(new AppError(AuthErrorCodes.UserNotFound, "Usuário não encontrado.", ErrorType.NotFound));
        ...
        var errors = result.Errors
            .Select(e => new AppError(AuthErrorCodes.ChangePasswordFailed, e.Description, ErrorType.Validation));
    ...
    public async Task<Result> ResetPasswordAsync(string email, string newPassword, CancellationToken ct = default)
    {
        ...
        var errors = result.Errors
            .Select(e => new AppError(AuthErrorCodes.ResetPasswordFailed, e.Description, ErrorType.Validation));
